...
inline void set_q(double qx, double qy, double qz)
{
	this->qx = qx;
	this->qy = qy;
	this->qz = qz;
...
inline void set_p(double px, double py, double pz)
{
	this->px = px;
	this->py = py;
	this->pz = pz;
...
inline tuple<double, double, double> get_q()
{
	return forward_as_tuple(qx, qy, qz);
}

inline tuple<double, double, double> get_p()
{
